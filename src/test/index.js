...
var fixturePublicKey = '91b3b113bb3150d8b7fe6a841be99ad25a062626453504c2bb9694a55a4a76f5'
var fixturePublicKeyStr = '0x' + fixturePublicKey
var fixturePublicKeyBuffer = Buffer.from(fixturePublicKey, 'hex')

var fixtureAddressKey = '9b80760513d7511ba56b1e843b1edb8b'
...
describe('.getPublicKey()', function () {
  it('should work', async function () {
    const publicKey = await fixtureWallet.getPublicKey()
...
describe('.getPublicKeyString()', function () {
  it('should work', async function () {
    const publicKeyString = await fixtureWallet.getPublicKeyString()
    assert.strictEqual(publicKeyString, fixturePublicKeyStr)
  })
})

describe('.getAddress()', function () {
  it('should work(await)', async function () {
    const address = await fixtureWallet.getAddress()
    assert.strictEqual(address.toString('hex'), fixtureAddressKey)
  })
  it('should work(then)', function () {
    fixtureWallet.getAddress().then(address => assert.strictEqual(address.toString('hex'), fixtureAddressKey))
  })
})

describe('.getAddressString()', function () {
  it('should work', async function () {
    assert.strictEqual(await fixtureWallet.getAddressString(), fixtureAddressKeyStr)
  })
})

describe('.getChecksumAddressString()', function () {
  it('should work', async function () {
    assert.strictEqual(await fixtureWallet.getChecksumAddressString(), ethUtil.toChecksumAddress(fixtureAddressKeyStr))
  })
})
...
describe('.generate()', function () {
  it('should generate an account(using then)', function () {
    Wallet.generate().then(value => { assert.strictEqual(value.getPrivateKey().length, 32) })
  });
  it('should generate an account(using async/await)', async function () {
    const wallet = await Wallet.generate()
    assert.strictEqual(wallet.getPrivateKey().length, 32);
  });
  it('should generate an account(using anymous)', async function () {
    (async() =>{
      console.log('start')
      const wallet = await Wallet.generate()
      console.log(wallet)
...
describe('.generateVanityAddress()', function () {
  it('should generate an account with 000 prefix (object)', async function () {
    ...
    var wallet = await Wallet.generateVanityAddress(/^000/)
    assert.strictEqual(wallet.getPrivateKey().length, 32)
